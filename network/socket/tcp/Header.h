...
class Socket : corteli::base::BaseObject, public std::enable_shared_from_this<Socket>
{
public:
	enum status
	{
	...
	Socket(boost::asio::io_service& ioService, bool enableDebugMessage = true);
	Socket(boost::asio::ip::tcp::socket socket, bool enableDebugMessage = true);
	int getStatus();
	int getError();
